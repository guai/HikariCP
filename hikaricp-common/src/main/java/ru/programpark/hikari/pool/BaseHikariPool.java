...
import static ru.programpark.hikari.pool.HikariMBeanElf.registerMBeans;
import static ru.programpark.hikari.pool.HikariMBeanElf.unregisterMBeans;
...
import static ru.programpark.hikari.util.IConcurrentBagEntry.STATE_REMOVED;
import static ru.programpark.hikari.util.UtilityElf.createInstance;
...
import ru.programpark.hikari.IConnectionCustomizer;
import ru.programpark.hikari.metrics.CodaHaleMetricsTracker;
import ru.programpark.hikari.metrics.CodahaleHealthChecker;
import ru.programpark.hikari.metrics.MetricsTracker;
import ru.programpark.hikari.proxy.ConnectionProxy;
import ru.programpark.hikari.proxy.IHikariConnectionProxy;
import ru.programpark.hikari.proxy.ProxyFactory;
import ru.programpark.hikari.util.ConcurrentBag;
import ru.programpark.hikari.util.DefaultThreadFactory;
...
public abstract class BaseHikariPool implements HikariPoolMBean, IBagStateListener
{
   ...
   private static final long ALIVE_BYPASS_WINDOW = Long.getLong("ru.programpark.hikari.aliveBypassWindow", 1000L);
   ...
   public BaseHikariPool(HikariConfig configuration, String username, String password)
   {
      ...
      this.addConnectionExecutor = UtilityElf
            .createThreadPoolExecutor(configuration.getMaximumPoolSize(), "HikariCP connection filler (pool " + configuration.getPoolName() + ")",
                                      configuration.getThreadFactory(), new ThreadPoolExecutor.DiscardPolicy());
      this.closeConnectionExecutor = UtilityElf
            .createThreadPoolExecutor(4, "HikariCP connection closer (pool " + configuration.getPoolName() + ")", configuration.getThreadFactory(),
                                      new ThreadPoolExecutor.CallerRunsPolicy());

      long delayPeriod = Long.getLong("ru.programpark.hikari.housekeeping.periodMs", TimeUnit.SECONDS.toMillis(30L));
      ...
      UtilityElf.setRemoveOnCancelPolicy(houseKeepingExecutorService);
   ...
   public final ConnectionProxy getConnection(final long hardTimeout) throws SQLException
   {
      ...
      final MetricsTracker.MetricsContext metricsContext = (isRecordMetrics ? metricsTracker.recordConnectionRequest(start) : MetricsTracker.NO_CONTEXT);
      ...
      throw new SQLTimeoutException(String.format("Timeout after %dms of waiting for a connection.", UtilityElf.elapsedTimeMs(start)), lastConnectionFailure.getAndSet(null));
   }
   ...
   public final void shutdown() throws InterruptedException
   {
      if (poolState != POOL_SHUTDOWN) {
         ...
         final ExecutorService assassinExecutor = UtilityElf
               .createThreadPoolExecutor(configuration.getMaximumPoolSize(), "HikariCP connection assassin", configuration.getThreadFactory(),
                                         new ThreadPoolExecutor.CallerRunsPolicy());
         final long start = System.currentTimeMillis();
         do {
            softEvictConnections();
         ...
         while (getTotalConnections() > 0 && UtilityElf.elapsedTimeMs(start) < TimeUnit.SECONDS.toMillis(5));
