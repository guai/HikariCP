...
public class ConcurrentBag<T extends IConcurrentBagEntry>
{
      ...
      if (!synchronizer.hasQueuedThreads()) {
         ...
         if (list == null) {
            ...
            for (int i = list.size() - 1; i >= 0; i--) {
               ...
               if (bagEntry != null && bagEntry.state().compareAndSet(IConcurrentBagEntry.STATE_NOT_IN_USE, IConcurrentBagEntry.STATE_IN_USE)) {
                  return (T) bagEntry;
               }
            }
         }
      ...
      do {
         ...
         do {
            ...
            for (final T bagEntry : sharedList) {
               if (bagEntry.state().compareAndSet(IConcurrentBagEntry.STATE_NOT_IN_USE, IConcurrentBagEntry.STATE_IN_USE)) {
                  return bagEntry;
               }
            }
   ...
   {
      if (bagEntry.state().compareAndSet(IConcurrentBagEntry.STATE_IN_USE, IConcurrentBagEntry.STATE_NOT_IN_USE)) {
         final ArrayList<WeakReference<IConcurrentBagEntry>> list = threadList.get();
         if (list != null) {
   ...
   {
      if (!bagEntry.state().compareAndSet(IConcurrentBagEntry.STATE_IN_USE, IConcurrentBagEntry.STATE_REMOVED) && !bagEntry.state().compareAndSet(
            IConcurrentBagEntry.STATE_RESERVED, IConcurrentBagEntry.STATE_REMOVED) && !closed) {
   ...
   public List<T> values(final int state)
   {
      ...
      if (state == IConcurrentBagEntry.STATE_IN_USE || state == IConcurrentBagEntry.STATE_NOT_IN_USE) {
   ...
   {
      return bagEntry.state().compareAndSet(IConcurrentBagEntry.STATE_NOT_IN_USE, IConcurrentBagEntry.STATE_RESERVED);
   }
   ...
   public void unreserve(final T bagEntry)
   {
      ...
      if (bagEntry.state().compareAndSet(IConcurrentBagEntry.STATE_RESERVED, IConcurrentBagEntry.STATE_NOT_IN_USE)) {
