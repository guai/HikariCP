...
import ru.programpark.hikari.util.ClassLoaderUtils;
...
public final class JavassistProxyFactory
{
   ...
   HashMap<Integer, Class> classesByHashCode = new HashMap<Integer, Class>();
...
//			sb.append("	invoked(\"method\", $args);\n");
   ...
   {
      CtClass proxyCt = classPool.getCtClass("ru.programpark.hikari.proxy.ProxyFactory");
      ...
      StringBuilder sb = new StringBuilder("{\n         switch ($1) {\n");
      for (Map.Entry<Integer, Class> entry : classesByHashCode.entrySet())
         sb.append("   case ").append(entry.getKey()).append(":\n      return ").append(entry.getValue().getName()).append(".class;\n");
      sb.append("   default:\n      throw new ClassNotFoundException(\"Class for hashCode = \" + $1 + \" not found\");\n   }\n}");

      CtMethod method = proxyCt.getMethod("classByHashCode", "(I)Ljava/lang/Class;");
   ...
   private <T> Class<T> generateProxyClass(Class<T> primaryInterface, Class<?> superClass, MethodBodyGenerator methodBodyGenerator) throws Exception
   {
      ...
      Class resultClass = targetCt.toClass(classPool.getClassLoader(), getClass().getProtectionDomain());
      classesByHashCode.put(resultClass.getName().hashCode(), primaryInterface);
      return resultClass;
   }
